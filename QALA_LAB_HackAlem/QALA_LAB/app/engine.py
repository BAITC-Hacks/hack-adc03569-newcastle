...
def _error(code: str, kk: str, ru: str) -> dict[str, Any]:
    return {"code": code, "message": {"kk": kk, "ru": ru}}
...
def validate(decisions: Any, *, complete: bool = True) -> dict[str, Any]:
    ...
    if not isinstance(decisions, list):
        return {"valid": False, "complete": False, "cost": 0, "remaining": 100,
                "count": 0, "category_counts": {}, "errors": [_error("TYPE", "Шешімдер тізім болуы керек.", "Решения должны быть массивом.")]}
    if len(decisions) > 5 or (complete and len(decisions) != 5):
        errors.append(_error("COUNT", "Дәл 5 шара таңдаңыз.", "Выберите ровно 5 мероприятий."))
    if len(decisions) > 50:
        return {"valid": False, "complete": False, "cost": 0, "remaining": 100,
    ...
    for item in decisions:
        if not isinstance(item, dict) or not set(item).issubset({"measure_id", "district_id"}):
            errors.append(_error("SCHEMA", "Шараның JSON құрылымы дұрыс емес.", "Неверная JSON-структура мероприятия."))
            continue
        mid = item.get("measure_id")
        if not isinstance(mid, str) or mid not in MEASURES:
            errors.append(_error("MEASURE", "Каталогта жоқ шара таңдалған.", "Выбрано неизвестное мероприятие."))
            continue
        m = MEASURES[mid]
        if mid in seen:
            errors.append(_error("DUPLICATE", f"{mid}: бір шараны қайталауға болмайды.", f"{mid}: повторное использование запрещено."))
        seen.add(mid)
        parsed[mid] = item
        cost += m["cost"]
        ...
        if m["scope"] == "city":
            if "district_id" in item:
                errors.append(_error("CITY_SCOPE", f"{mid}: қалалық шараға аудан көрсетілмейді.", f"{mid}: у городской меры район не указывается."))
        elif not isinstance(district, str) or district not in DISTRICTS:
            errors.append(_error("DISTRICT", f"{mid}: каталогтағы бір ауданды таңдаңыз.", f"{mid}: выберите один район из каталога."))
    if cost > 100:
        errors.append(_error("BUDGET", "100 бірлік бюджеттен асып кетті.", "Превышен бюджет 100 единиц."))
    if any(count > 2 for count in categories.values()):
        errors.append(_error("CATEGORY", "Бір бағыттан ең көбі 2 шара таңдауға болады.", "Не более 2 мер из одного направления."))
    for rule in DATA["incompatibilities"]:
        ...
        if a in parsed and b in parsed:
            ...
            if rule["scope"] == "any" or same:
                errors.append(_error("CONFLICT", f"{a} + {b}: бұл үйлесімге ережелер тыйым салады.", f"{a} + {b}: сочетание запрещено правилами."))
